...
<div class="flex justify-between items-center mb-5 ">
    <form action={{route('noticias.buscar')}} method="POST">
        @csrf
        <label for="titulo" class="inline text-gray-700 text-2xl">Buscar noticia:</label>
        ...
        <input id="autor" type="text" class="inline p-3 bg-white rounded form-input  @error('autor') is-invalid @enderror" name="autor" value="{{isset($noticia->autor)? ($noticia->autor->direccion): old('autor')}}" placeholder="Autor" autocomplete="autor" autofocus>
        @error('autor')
            <span class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mt-5" role="alert">
                <strong>{{ $message }}</strong>
            </span>
